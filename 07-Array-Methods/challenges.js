...
//   // const fullDogList = [...dogsJuliaCorrect, ...dogsKate];
//   const fullDogList = dogsJuliaCorrect.concat(dogsKate)

//   fullDogList.forEach(function (year, i) {
//     year >= 3 ? console.log(`Dog number ${i + 1} is an adult, and is ${year} years old`) : console.log(`Dog number ${i + 1} is still a puppy 🐶`)
//   })
...
const sarahDog = dogs.find(dog => dog.owners.some(e => e === 'Sarah'));
...
console.log(`Sarah dogs eats ${sarahDog.curFood > sarahDog.recommendedFood * 1.1 || !(sarahDog.curFood < sarahDog.recommendedFood * 0.9) ? 'too much!' : 'too less!'}`);
...
// dogs.forEach(dog => dog.curFood > dog.recommendedFood * 1.1 || !(dog.curFood < dog.recommendedFood * 0.9) ? ownersEatTooMuch.push(dog.owners) : ownersEatTooLittle.push(dog.owners))

dogs.map(dog => dog.curFood > dog.recommendedFood * 1.1 || !(dog.curFood < dog.recommendedFood * 0.9) ? ownersEatTooMuch.push(...dog.owners) : ownersEatTooLittle.push(...dog.owners))
...
const okay = [];
dogs.map(dog => dog.curFood > dog.recommendedFood * 0.9 && dog.curFood < dog.recommendedFood * 1.1 ? okay.push(dog) : null);
